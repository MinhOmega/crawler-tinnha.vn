...
def crawl_wordpress_products(base_url, max_workers=10):
    ...
    # Progress bar for scraping pages
    ...
    # Fetch product details concurrently using ThreadPoolExecutor with progress bar
    print(f"Fetching details for {len(all_product_data)} products concurrently...")
    detailed_products = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        for product in tqdm(executor.map(lambda p: scrape_product_details(p['product_url'], p['product_id']), all_product_data), total=len(all_product_data), desc="Fetching Product Details"):
