...
import time  # Import the time module for logging execution time
...
def crawl_wordpress_products(base_url):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        detailed_products = list(executor.map(lambda p: scrape_product_details(p['product_url']), all_product_data))
